...
plt.plot(x, ious, alpha=alpha, linewidth=1, label="Center Error")
plt.plot(x, model_ious, alpha=alpha, linewidth=1, label="Center Error Model-ResNet18(raw)")

plt.xlabel("Time (frame/step)")
plt.ylabel("Center Error")
plt.title("Center Error over time")
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()

graph_path = f'./plots/CENTER_ERROR_session{session}_participant{participant}.png'
